...
test("Usage quota observability uses independent durable routing analytics", async () => {
  ...
  expect(quota).toContain("data.sequentialRoutes === 0");
  expect(quota).toContain('t("usage.quota.status.observeOnly")');
});
...
test("Usage quota settles while primary usage remains pending", async () => {
  ...
  try {
    ...
    while (!(container.textContent ?? "").includes("No route/session warning occurrences in scanned history.")) {
...
test("usage workspace i18n keys exist in every locale", async () => {
  ...
  for (const locale of locales) {
    ...
    expect(dict).toContain('"usage.quota.status.observeOnly":');
