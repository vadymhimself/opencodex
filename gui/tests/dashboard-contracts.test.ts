...
test("quota state distinguishes physical-attempt telemetry gaps and historical warnings", () => {
  ...
  expect(historical).toContain("1 route/session warning occurrence(s) in scanned history");
  ...
  expect(alertWithPartialTelemetry).toContain("1 route/session warning occurrence(s) in scanned history");
  ...
  expect(stale).not.toContain("No route/session warning occurrences in scanned history.");
