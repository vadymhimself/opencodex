...
describe("routing analytics (RI-03)", () => {
  ...
  test("prices legacy root usage on its final physical attempt", async () => {
    ...
    const expected = estimateComboCost([{
      ...physical,
      usage,
      usageStatus: "reported",
    }]);
    expect(expected).not.toBeNull();
    appendUsageEntry(entry("legacy-root-cost", {
      timestamp: 1,
      provider: "combo",
      model: "policy/fast",
      status: 200,
  ...
  test("combo pricing ignores duplicate, local, and unsent attempts", async () => {
    const physical = attempt({
      provider: "openai-apikey",
      model: "gpt-5.6-sol",
      usage: { inputTokens: 1_000, outputTokens: 100 },
    });
    const expected = estimateComboCost([physical]);
    expect(expected).not.toBeNull();
    appendUsageEntry(entry("combo-physical-cost", {
      timestamp: 1,
      provider: "combo",
      model: "policy/fast",
      status: 200,
  ...
  test("does not attribute root usage when physical attempt usage is present but invalid", async () => {
    appendUsageEntry(entry("invalid-attempt-usage", {
      timestamp: 1,
      status: 200,
      durationMs: 10,
      usage: { inputTokens: 250, outputTokens: 1, cacheReadInputTokens: 200 },
      attempts: [
        attempt({
          provider: "first",
          usage: {
    ...
    expect(result.physicalBreakdown.find(row => row.provider === "final")?.attemptUsage.inclusiveInputTokens).toBe(0);
  });
  ...
  test("counts recovery occurrences and excludes duplicate and locally answered attempts", async () => {
    ...
    const row = {
      ...
      attempts: [
        ...
        attempt({ ordinal: 3, provider: "unsent", sendCount: 0, usageStatus: "unreported" }),
      ],
    };
  ...
  test("treats explicit all-invalid and all-filtered attempt arrays as zero physical attempts", async () => {
    const rows = [
      {
        ...entry("invalid-attempt-array", {
          timestamp: 1,
          status: 200,
          durationMs: 10,
          usage: { inputTokens: 100, outputTokens: 10, cacheReadInputTokens: 40 },
        }),
        attempts: [null, { ordinal: "bad" }],
      },
      {
        ...entry("filtered-attempt-array", {
          timestamp: 2,
          status: 200,
          durationMs: 10,
          usage: { inputTokens: 200, outputTokens: 20, cacheReadInputTokens: 80 },
        }),
        attempts: [
          attempt({ locallyAnswered: true }),
          attempt({ ordinal: 2, sendCount: 0 }),
        ],
      },
    ];
    appendFileSync(usageLogPath(), `${rows.map(row => JSON.stringify(row)).join("\n")}\n`);
  ...
  test("recovers combined read and write tokens from legacy cache totals", async () => {
    appendUsageEntry(entry("legacy-cache", {
      timestamp: 1,
      status: 200,
      durationMs: 10,
      attempts: [attempt({
  ...
  test("emits route-sequential red alerts in timestamp and request-id order", async () => {
    ...
    expect(result.sequentialRoutes).toBe(3);
    expect(result.warmedRoutes).toBe(1);
    expect(result.redAlertsPartial).toBe(false);
  });

  test("cache creation alone warms an Anthropic route", async () => {
    const add = (requestId: string, timestamp: number, usage: PersistedUsageAttempt["usage"]) =>
      appendUsageEntry(entry(requestId, {
        timestamp,
        status: 200,
        durationMs: 10,
    ...
    const result = await computeRoutingAnalytics({});
    const alerts = result.redAlerts
      .filter(alert => alert.requestId === "bad-cache")
      .map(alert => alert.kind)
      .sort();
  ...
  test("does not mistake a fully reused prefix plus a large tool result for cache loss", async () => {
    const add = (requestId: string, timestamp: number, usage: PersistedUsageAttempt["usage"]) =>
      appendUsageEntry(entry(requestId, {
        timestamp,
        status: 200,
        durationMs: 10,
        conversationId: "growing-conversation",
        attempts: [attempt({
          provider: "anthropic-paaaaaa",
          model: "claude-opus-5",
          usage,
    ...
    add("warm-prefix", 1, {
      inputTokens: 4_539,
      outputTokens: 1,
      cacheCreationInputTokens: 4_537,
    });
    add("large-tool-result", 2, {
      inputTokens: 25_657,
      outputTokens: 1,
      cacheReadInputTokens: 4_537,
      cacheCreationInputTokens: 21_118,
    });

    const alerts = (await computeRoutingAnalytics({})).redAlerts
      .filter(alert => alert.requestId === "large-tool-result");
    expect(alerts).toEqual([]);
  });

  test("detects a full-prefix rewrite despite legitimate prompt growth", async () => {
    const add = (requestId: string, timestamp: number, usage: PersistedUsageAttempt["usage"]) =>
      appendUsageEntry(entry(requestId, {
        timestamp,
        status: 200,
        durationMs: 10,
        conversationId: "rewritten-conversation",
        attempts: [attempt({
          provider: "anthropic-paaaaaa",
          model: "claude-opus-5",
          usage,
    ...
    add("initial-write", 1, {
      inputTokens: 931_882,
      outputTokens: 1,
      cacheCreationInputTokens: 931_880,
    });
    add("full-rewrite", 2, {
      inputTokens: 966_539,
      outputTokens: 1,
      cacheCreationInputTokens: 966_537,
    });

    const alerts = (await computeRoutingAnalytics({})).redAlerts
      .filter(alert => alert.requestId === "full-rewrite")
      .map(alert => alert.kind)
      .sort();
    expect(alerts).toEqual([
  ...
  test("keeps one caller-side lineage rewrite isolated when its successor reuses cache", async () => {
    const add = (requestId: string, timestamp: number, usage: PersistedUsageAttempt["usage"]) =>
      appendUsageEntry(entry(requestId, {
        timestamp,
        status: 200,
        durationMs: 10,
        conversationId: "source-lineage-change",
        attempts: [attempt({
          provider: "anthropic-paaaaaa",
          model: "claude-opus-5",
          usage,
    ...
    add("warmed-lineage", 1, {
      inputTokens: 28_395,
      outputTokens: 1,
      cacheReadInputTokens: 27_927,
      cacheCreationInputTokens: 466,
    });
    add("changed-toolset", 2, {
      inputTokens: 28_110,
      outputTokens: 1,
      cacheCreationInputTokens: 28_108,
    });
    add("stable-new-lineage", 3, {
      inputTokens: 28_508,
      outputTokens: 1,
      cacheReadInputTokens: 28_108,
      cacheCreationInputTokens: 398,
    });

    const alerts = (await computeRoutingAnalytics({})).redAlerts;
    expect(alerts
      .filter(alert => alert.requestId === "changed-toolset")
      .map(alert => alert.kind)
      .sort()).toEqual([
      "falling-cache-read",
      "high-cache-write-after-warmup",
    ...
    expect(alerts.filter(alert => alert.requestId === "stable-new-lineage")).toEqual([]);
  });

  test("keeps interleaved cache lineages independent within one Claude conversation", async () => {
    const add = (requestId: string, timestamp: number, usage: PersistedUsageAttempt["usage"]) =>
      appendUsageEntry(entry(requestId, {
        timestamp,
        status: 200,
        durationMs: 10,
        conversationId: "shared-parent-conversation",
        attempts: [attempt({
          provider: "anthropic-paaaaaa",
          model: "claude-opus-5",
          usage,
    ...
    add("lineage-b", 2, {
      inputTokens: 700_002,
      outputTokens: 1,
      cacheCreationInputTokens: 700_000,
    });
    add("lineage-a-resumed", 3, {
      inputTokens: 100_402,
      outputTokens: 1,
      cacheReadInputTokens: 100_000,
      cacheCreationInputTokens: 400,
    });
    add("lineage-b-resumed", 4, {
      inputTokens: 700_502,
      outputTokens: 1,
    ...
    const alerts = (await computeRoutingAnalytics({})).redAlerts;
    expect(alerts.filter(alert => alert.requestId === "lineage-a-resumed")).toEqual([]);
    expect(alerts.filter(alert => alert.requestId === "lineage-b-resumed")).toEqual([]);
  });

  test("expires Anthropic warm-up state after the prompt-cache TTL", async () => {
    const add = (
      requestId: string,
      timestamp: number,
      usage: PersistedUsageAttempt["usage"],
      ...
      attempts: [attempt({
        ...
        model: "claude-opus-5",
        usage,
        ...({ promptCacheTtlMs: 5 * 60_000 } as Partial<PersistedUsageAttempt>),
      })],
    }));
    ...
    add("warm", 1, {
      inputTokens: 800_229,
      outputTokens: 1,
      cacheReadInputTokens: 799_819,
      cacheCreationInputTokens: 408,
    });
    add("post-idle-rewrite", 568_201, {
      inputTokens: 808_955,
      outputTokens: 1,
      cacheCreationInputTokens: 808_953,
    });
    add("reuse-rewritten-prefix", 568_202, {
      inputTokens: 810_595,
      outputTokens: 1,
      cacheReadInputTokens: 808_953,
      cacheCreationInputTokens: 1_640,
    });
    add("raw-before-idle", 1, { inputTokens: 120_001, outputTokens: 1 }, "raw-conversation");
    add("raw-after-idle", 568_201, { inputTokens: 120_002, outputTokens: 1 }, "raw-conversation");

    const result = await computeRoutingAnalytics({});
    expect(result.redAlerts).toEqual([]);
    expect(result.sequentialRoutes).toBe(2);
    expect(result.warmedRoutes).toBe(1);
  });

  test("keeps a one-hour Anthropic route warm beyond the default five-minute TTL", async () => {
    const add = (
      requestId: string,
      timestamp: number,
      usage: PersistedUsageAttempt["usage"],
      ...
      status: 200,
      durationMs: 10,
      conversationId: "long-cache-conversation",
      attempts: [attempt({
        provider: "anthropic-paaaaaa",
        model: "claude-opus-5",
        usage,
        ...({ promptCacheTtlMs: 60 * 60_000 } as Partial<PersistedUsageAttempt>),
      })],
    }));

    add("warm-long-cache", 1, {
      inputTokens: 800_229,
      outputTokens: 1,
      cacheReadInputTokens: 799_819,
      cacheCreationInputTokens: 408,
    });
    add("leak-before-long-cache-expiry", 568_201, {
      inputTokens: 808_955,
      outputTokens: 1,
      cacheCreationInputTokens: 808_953,
    });
    ...
    const alerts = (await computeRoutingAnalytics({})).redAlerts
      .filter(alert => alert.requestId === "leak-before-long-cache-expiry")
      .map(alert => alert.kind);
    expect(alerts).toContain("high-cache-write-after-warmup");
  });

  test("orders sequential alerts by first physical send rather than request completion", async () => {
    const add = (
      requestId: string,
      timestamp: number,
      firstSendAt: number,
      ...
      status: 200,
      durationMs: 10,
      conversationId: "overlapping-conversation",
      attempts: [attempt({
        provider: "anthropic-paaaaaa",
        model: "claude-opus-5",
        usage: { inputTokens, outputTokens: 1 },
        ...({ firstSendAt } as Partial<PersistedUsageAttempt>),
      })],
    }));

    add("sent-first-finished-last", 1_000, 100, 110_000);
    add("sent-second-finished-first", 500, 200, 120_000);

    const alerts = (await computeRoutingAnalytics({})).redAlerts
      .filter(alert => alert.kind === "consecutive-high-raw-input");
    expect(alerts).toEqual([
      expect.objectContaining({ requestId: "sent-second-finished-first", previousValue: 110_000 }),
    ]);
  });

  test("does not apply Anthropic cache thresholds to OpenAI routes", async () => {
    const add = (
      requestId: string,
      timestamp: number,
      usage: PersistedUsageAttempt["usage"],
      overrides: Partial<PersistedUsageAttempt> = {},
    ) => appendUsageEntry(entry(requestId, {
      timestamp,
      status: 200,
      durationMs: 10,
      conversationId: "conversation",
      attempts: [attempt({ provider: "openai", model: "gpt-5.6-terra", usage, ...overrides })],
    }));

    add("raw-1", 1, { inputTokens: 110_000, outputTokens: 1 });
    add("raw-2", 2, { inputTokens: 120_000, outputTokens: 1 });
    add("warm", 3, { inputTokens: 10_000, outputTokens: 1, cacheReadInputTokens: 9_500 });
    add("low-read", 4, {
      inputTokens: 20_000,
      outputTokens: 1,
      cacheReadInputTokens: 1_000,
      cacheCreationInputTokens: 11_000,
    }, { sendCount: 2, recoveryKinds: ["connection-reset"] });

    const result = await computeRoutingAnalytics({});
    expect(result.redAlerts.map(alert => alert.kind).sort()).toEqual(["recovery", "repeated-send"]);
    expect(result.sequentialRoutes).toBe(0);
    expect(result.warmedRoutes).toBe(0);
  });

  test("applies Anthropic cache thresholds to custom-named Anthropic adapters", async () => {
    for (const [requestId, timestamp] of [["raw-1", 1], ["raw-2", 2]] as const) {
      appendUsageEntry(entry(requestId, {
        timestamp,
        status: 200,
        durationMs: 10,
        ...
        attempts: [attempt({
          ...
          model: "claude-opus-5",
          adapter: "anthropic",
          usage: { inputTokens: 110_000 + timestamp, outputTokens: 1 },
        })],
      }));
    }
    ...
    expect(result.redAlerts).toEqual([
      expect.objectContaining({
        kind: "consecutive-high-raw-input",
        requestId: "raw-2",
        provider: "custom-name",
      }),
    ]);
    expect(result.sequentialRoutes).toBe(1);
  });
