    ...
    switch (name) {
      ...
      case "content_block_delta": {
        ...
        if (delta.type === "text_delta" && typeof delta.text === "string") {
          ...
          const citations = Array.isArray(openBlock.citations) ? openBlock.citations : [];
          const citation = structuredClone(delta.citation);
          const citationBytes = Buffer.byteLength(JSON.stringify(citation))
            + (citations.length === 0 ? 2 : 1);
          const reservation = translatorBudget.reserveTransient(citationBytes, { kind: "retained_collectors" });
          citations.push(citation);
          reservation.commitRetained();
          openBlock.citations = citations;
          openCitationBytes += citationBytes;
